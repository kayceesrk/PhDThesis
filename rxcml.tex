...
between performing (a) a local receive (\cf{localComm}), (b) a remote send
(\cf{remoteSend}), or (c) a remote receive (\cf{remoteRecv}). We have extended
...
enabled if \cf{lOut} is not empty. Otherwise, it is a \cf{neverEvt()} which
will never be picked in a choice. If \cf{lOut} is not empty, then the
...
author. The daemon uses a \cf{timeoutManager} (lines 28-30) to periodically
fetch updates from the user interface thread. The daemon then receives local
updates \cf{lOps}, if any, from the channel \cf{lc} (lines 19).
...
\cf{lOps}, to yield an \cf{rIn'} (line 20) that considers the remote updates in
