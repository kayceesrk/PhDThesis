...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{Figures/LocalCollector.pdf}
...
pointer lift (pointer op, bool isLocalMany) {
  ...
  foreach o in imSet
		foreachObjptrInObject(o,fixFwdPtr);
  frontier = getLocalHeapFrontier();
  if (isLocalMany)
    foreachObjptrInRange
			(getSessionStart(), &frontier, fixFwdPtr);
...
forwarding pointers (Line 21). Since we do not track references from the stack
...
references to forwarded objects (Lines 22--23).
...
forwarded objects to point to their shared heap counterparts (lines 25--27).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{Figures/SMC_Collector.pdf}
...
\texttt{smcRelease()}, to flush any outstanding writes to the memory (Line 8 in
